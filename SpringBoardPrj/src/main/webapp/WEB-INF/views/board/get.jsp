...
<!-- 2. body태그 하단에 <script>태그 작성 후 var bno = ${vo.bno}로 글 번호를
	받은 다음 function getAllList()를 test.jsp에서 복분해서 게시물 별 페이지에서
	잘 작동하는지 확인도 해주세요. -->
<style>
	#modDiv {
		width:300px;
...
<body>
	...
	<script>
		...
		$("#replies").on("click", ".replyLi button", function(){
			...
			var reply = $(this).siblings(".reply").text(); // li태그 글씨만 얻기.
		...
		function getAllList(){
			$.getJSON("/replies/all/" + bno, function(data){
				...
				$(data).each(function(){
					...
					+ this.replyer + "</string> - " + formattedTime + "<br>"
					+ "<div class='reply'>" + this.reply + "</div>"
					+ "<button type='button' class='btn btn-ingo'>수정/삭제</button>"
					+ "</div>";
				});
